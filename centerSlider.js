...
"use strict";
    ...
    $.fn.centerSlider = function(options) {
        return this.each(function() {
            ...
            // init variables
            var slideWidth, contWidth, spacerWidth, currentSlide, translate;
            ...
            var settings = $.extend($.fn.centerSlider.defaults, options);
            ...
            $slides
                .css('float', 'left')
                .first()
                .addClass('centerslide-active')
                .next(settings.slide)
                .addClass('centerslide-next');

            // wrap slides, cache wrapper
            var $wrapper = $slides.wrapAll('<div class="centerslide-wrapper"></div>').parent();

            // set height of wrapper, add beginning and end spacers
            $wrapper
                .css('height', '100%')
                .prepend($('<div class="centerslide-spacer"></div>').css({ float: 'left', height: '100%' }))
                .append($('<div class="centerslide-spacer"></div>').css({ float: 'left', height: '100%' }));

            // size the container based on speed setting
            var sizeSlides = function() {

                contWidth = $container.width();
                var objWidth = $container.find( settings.object ).first().width();

                // formula to solve slide width
                slideWidth = ((contWidth - (settings.peekWidth * 2) - objWidth) / 2) + objWidth;

                // solve spacer width
                spacerWidth = (contWidth - slideWidth) / 2;

                // set slide width
                $slides.width(slideWidth);

                // set spacer width
                $container.find('.centerslide-spacer').width(spacerWidth);

                // set container width
                $wrapper
                    .css({ minWidth: ((slideWidth * $slides.length) + (spacerWidth * 2)) })
                    .velocity({ translateX: 0 - (currentSlide * slideWidth) }, 0);
            ...
            // run twice in case object width is relative (weird, I know)
            sizeSlides();
            sizeSlides();
            ...
            $container.on('cs-prev', function(){
                ...
                var $in = $slides.eq((currentSlide - 1));
                var $out = $slides.eq(currentSlide);
                if ( typeof settings.before === 'function' ) settings.before($in, $out);

                // set classes
                $slides.removeClass('centerslide-active centerslide-next centerslide-prev');
                $in.addClass('centerslide-active');
                $in.next(settings.slide).addClass('centerslide-next');
                $in.prev(settings.slide).addClass('centerslide-prev');
            ...
            // set key listeners if arrowKeys is set
